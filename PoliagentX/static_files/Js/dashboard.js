...
function renderTable(selectedIndicators, indicatorData) {
    ...
    if (selectedIndicators.length === 1) {
        ...
        dataObj.x.forEach((t, idx) => {
            tbody.innerHTML += `<tr>
                <td>${t}</td>
                <td style="color:${dataObj.row.color}">${dataObj.y[idx].toFixed(2)}</td>
                <td>${dataObj.goal[idx].toFixed(2)}</td>
                <td><div class="w-4 h-4 rounded-full" style="background:${dataObj.row.color}"></div></td>
            </tr>`;
        });
